    ...
    $.fn.mapsed = function (options) {
            ...
            navigator.geolocation.getCurrentPosition(
                function (geoPos) {
                    ...
                    if (so && so.geoSearch && so.geoSearch.length > 0) {
                        ...
                        var search = so.geoSearch.replace(
                            "{POSITION}",
                            newLocation
                        );
        ...
        function gmMapLoaded() {
            ...
            if (gmToolBtns.length > 0) {
                ...
                $(_mapContainer.find(".mapsed-control-button")).css(
                    "line-height",
                    gmLineHeight
                );
            }
        ...
        function getPlaceDetails(forMarker, callback) {
            if (!forMarker.details) return;
            if (!forMarker.details.reference) return;

            var request = {
                reference: forMarker.details.reference,
            };
            _placesApi.getDetails(request, function (placeDetails, status) {
        ...
        function hideEmpty(model, $vw) {
            ...
            $vw.find(".mapsed-name")
                .parent()
                .toggle(model.name && model.name.length > 0);
            $vw.find(".mapsed-street").toggle(
                model.street && model.street.length > 0
            );
            $vw.find(".mapsed-town").toggle(
                model.town && model.town.length > 0
            );
            $vw.find(".mapsed-area").toggle(
                model.area && model.area.length > 0
            );
            $vw.find(".mapsed-postCode").toggle(
                model.postCode && model.postCode.length > 0
            );
            $vw.find(".mapsed-country").toggle(
                model.country && model.country.length > 0
            );
            ...
            if (settings.onSelect || settings.onSave || settings.onDelete) {
                ...
                if (model.markerType == "google") canEdit = false;
        ...
        function addSearch() {
            ...
            var so = settings.searchOptions,
                html =
                    "<input type='text' id='" +
                    id +
                    "' class='mapsed-searchbox' autocomplete='off' ";
            html += "placeholder='";
            if (so.enabled && so.placeholder) html += so.placeholder;
            else html += "Search ...";
            html += "' ";
            ...
            gm.event.addListener(_gmSearchBox, "places_changed", function () {
        ...
        function addCloseButton() {
            ...
            $("body").on("keyup", function (evt) {
                evt.preventDefault();
                if (evt.which == 27 /*ESCape*/) onCloseEvent(evt);
            });
        } // addCloseButton
        ...
        function createControlButton(
            ...
            if (buttonText && buttonText.length > 0) {
                ...
                tooltip =
                    arrSplit.length > 1 ? " title='" + arrSplit[1] + "'" : "";
            }
        ...
        function closeTooltips() {
            if (_markers && _markers.length > 0) {
                for (var i = 0; i < _markers.length; i++) {
                    ...
                    if (current.tooltip) current.tooltip.close();
                }
            }
        } // closeTooltips
        ...
        function attachTooltip(forMarker) {
            ...
            var item = $(
                "<div id='mapsed-" +
                    newId +
                    "' class='mapsed-root'>" +
                    "<input type='hidden' class='mapsed-lat' value='" +
                    forMarker.position.lat() +
                    "' />" +
                    "<input type='hidden' class='mapsed-lng' value='" +
                    forMarker.position.lng() +
                    "' />" +
                    "<input type='hidden' class='mapsed-can-edit' value='" +
                    d.canEdit +
                    "' />" +
                    "<input type='hidden' class='mapsed-reference' value='" +
                    d.reference +
                    "' />" +
                    "<input type='hidden' class='mapsed-user-data' value='" +
                    d.userData +
                    "' />" +
                    "<input type='hidden' class='mapsed-marker-type' value='" +
                    forMarker.markerType +
                    "' />" +
                    getViewTemplate() +
                    getEditTemplate() +
                    "</div>"
        ...
        function showTooltip(inRwMode) {
            ...
            if (!model.isLoaded) {
                ...
                getPlaceDetails(marker, function (forM) {
        ...
        function getViewTemplate() {
            ...
            var html =
                ...
                "<td colspan='3'>" +
                "<button class='mapsed-select-button'>" +
                settings.ActionButtons.Select +
                "</button>" +
                "<button class='mapsed-edit-button'>" +
                settings.ActionButtons.Edit +
                "</button>" +
                "<button class='mapsed-delete-button'>" +
                settings.ActionButtons.Delete +
                "</button>" +
                "</td>" +
                "</tr>" +
                "</table>";
        ...
        function getEditTemplate() {
            var html =
                ...
                "<button class='mapsed-save-button'>" +
                settings.ActionButtons.Save +
                "</button>" +
        ...
        function addInitialPlaces() {
            ...
            if ($.isArray(settings.showOnLoad)) places = settings.showOnLoad;
            else places.push(settings.showOnLoad);
        ...
        function normaliseFormattedAddress(details, src) {
            ...
            // pure guess!
            if (elements.length >= 1) details.street = $.trim(elements[0]);
            if (elements.length >= 2) details.town = $.trim(elements[1]);
            if (elements.length >= 3) details.area = $.trim(elements[2]);
            if (elements.length >= 4) details.postCode = $.trim(elements[3]);
            if (elements.length >= 5) details.country = $.trim(elements[4]);
        }
        ...
        if (!this.length) {
            ...
            if (!_mapContainer.length) {
                _mapContainer = $(
                    "<div id='mapsed-full-window' class='mapsed-full-window'></div>"
                );
